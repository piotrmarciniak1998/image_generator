...
import os
import pickle
from unused_category import Category
...
for category in category_list:
    ...
    for index, obj_file in enumerate(obj_files):
        ...
        # inertia_ixx = f"{10.0/6.0 * scale * scale * scale}"
        # inertia_iyy = f"{10.0/6.0 * scale * scale * scale}"
        # inertia_izz = f"{10.0/6.0 * scale * scale * scale}"

        # mass_value = f"{scale * 100}"
        # inertia_ixx = f"{scale * 100}"
        # inertia_iyy = f"{scale * 100}"
        # inertia_izz = f"{scale * 100}"
        ...
        #     inertia.attrib["ixx"] = inertia_ixx
        #     inertia.attrib["iyy"] = inertia_iyy
        #     inertia.attrib["izz"] = inertia_izz
